...
def load_segmentation_model(checkpoint_path,application, device):
    """
    ...
    """
    if application == "change_detection":
        segmentation_model = UNetC(num_classes=1)  # Replace with your actual model class
    elif application == "modality_conversion":
        segmentation_model = UNet(n_classes=1)
    else:
        raise NotImplementedError(f"Application unknown {application}")

    ckpt = torch.load(checkpoint_path, map_location='cpu',weights_only=True)  # Load on CPU first
    ckpt = adjust_keys(ckpt, "norm", "bn")
...
def run_training(args):
    ...
    segmentation_model = load_segmentation_model(args.segmentation_model, application=args.application, device=device)

    model, diffusion = create_conditional_model_and_diffusion(
        **args_to_dict(args, model_and_diffusion_defaults().keys()),
        segmentation_model=segmentation_model, application=args.application
    )
    model.to(device)
    ...
    # Start training loop
    logger.log(f"Training for {args.application}")
...
def create_argparser():
    defaults = dict(
        ...
        max_grad_norm=1.0,  # Added gradient clipping parameter
    )
