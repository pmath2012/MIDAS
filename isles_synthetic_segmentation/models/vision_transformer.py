...
from models.utils import TwoLayerConv2d, Transformer, TransformerDecoder
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            self.resnet = resnet.resnet18(in_channels=input_nc, pretrained=pretrained,
                                          replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet34':
            self.resnet = resnet.resnet34(in_channels=input_nc, pretrained=pretrained,
                                          replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet50':
            self.resnet = resnet.resnet50(in_channels=input_nc, pretrained=pretrained,
                                          replace_stride_with_dilation=[False,True,True])
            expand = 4
        else:
...
# Single-input Vision Transformer, based on ViTSeg but for one input only
class ViT(ResNet):
    """
    ResNet backbone + Vision Transformer for single input segmentation/classification.
    """
    def __init__(self, in_channels, num_classes, with_pos, resnet_stages_num=5,
                 ...
                 with_decoder=True,
                 pretrained=False):
        super(ViT, self).__init__(in_channels, num_classes, backbone=backbone,
                                  resnet_stages_num=resnet_stages_num,
                                  if_upsample_2x=if_upsample_2x, pretrained=pretrained)
    ...
    def forward(self, x):
        ...
        if self.token_trans:
            tokens = self._forward_transformer(token)
        else:
            tokens = token
        ...
        if self.with_decoder:
            ...
            x = self._forward_simple_decoder(x, tokens)
        if not self.if_upsample_2x:
            x = self.upsamplex2(x)
        x = self.upsamplex4(x)
        x = self.classifier(x)
