...
class MSDataset(Dataset):
    def __init__(self, dataframe, root_dir, application="change_detection", transform=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.application == "change_detection":
            ...
            lesion_load = lesion_load / PERF_LOAD_MAX
            num_lesions = num_lesions / PERF_NUM_MAX
        else:
            raise NotImplementedError(f"Unknown Application {application}")

        # Apply transforms (including the image and mask transformations)
        if self.transform:
            image_t0, image_t1, mask = self.transform(image1, image2, mask)

        # Prepare the conditioning dictionary (out_dict)
        out_dict = {
            "ground_truth": mask,  # Mask tensor
            "t0": image_t0,  # Conditioning image tensor
        ...
        return image_t1, out_dict

# Function to split CSV and create training dataset (without validation)
def load_data(csv_file, root_dir, batch_size,application="change_detection", distributed=False, train=True):
    # Load CSV file
