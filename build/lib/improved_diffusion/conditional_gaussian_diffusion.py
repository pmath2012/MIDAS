...
class ConditionalGaussianDiffusion(SpacedDiffusion):
    def __init__(self, betas, model_mean_type,
            ...
            self.log_lambda_recon = th.nn.Parameter(th.tensor(0.01))  # For reconstruction loss
            self.log_lambda_seg = th.nn.Parameter(th.tensor(0.01))  
    ...
    def training_losses(self, model, x_start, t, noise=None, model_kwargs=None):
        ...
        if self.segmentation_model is not None:
            with th.no_grad():
                if self.application == "change_detection":
                    new_x = th.cat([image_t0, pred_xstart], dim=1)
                elif self.application == "modality_conversion":
                    new_x = pred_xstart
                else:
                    raise NotImplementedError(f"Method not implemented : {self.application}")
                predicted_mask = self.segmentation_model(new_x)
            # segmentation_loss =  F.mse_loss(predicted_mask, ground_truth)
            ...
            bce_loss = self._weighted_bce(predicted_mask, ground_truth)
            f_loss = self._fbeta_loss(predicted_mask, ground_truth)
            segmentation_loss = bce_loss + f_loss

            # total_loss += bce_loss + f_loss
            total_loss += (
            th.exp(-self.log_lambda_recon) * diffusion_loss + self.log_lambda_recon +
            th.exp(-self.log_lambda_seg) * segmentation_loss + self.log_lambda_seg
            )
        return {"loss": total_loss}


    def _apply_drop_condition(self, model_kwargs, drop_condition):
        # Removing the image instead of the mask
        if drop_condition:
            # set the image to random but keep the other variables as is for loss computation
            model_kwargs['t0'] = th.rand_like(model_kwargs.get('t0'))
        
        # set the mask as a set guidance to force model to learn the change 
    ...
    def _weighted_bce(self, predicted, ground_truth, cap=1000.0):
        weights = th.where(ground_truth > 0, cap, 1.0)
        return F.binary_cross_entropy_with_logits(predicted, ground_truth, weight=weights)

    def _fbeta_loss(self, predicted, ground_truth, beta=2):
        eps = 1e-7
        predicted = th.sigmoid(predicted)
        predicted = predicted.view(-1)
        ground_truth = ground_truth.view(-1)

        tp = (predicted*ground_truth).sum()
        fp = ((1-ground_truth)*predicted).sum()
        fn = (ground_truth*(1-predicted)).sum()

        prc = tp/(tp+fp+eps)
        rec = tp/(tp+fn+eps)

        fbeta = (1+beta**2)*(prc*rec)/((beta**2)*prc + rec + eps )
        return 1 - fbeta
