...
def create_conditional_model_and_diffusion(
    ...
    application="change_detection", # choose between Change detection (2 inputs for Segmentation model) vs modality conversion (1 input for segmentation)
):
    """
...
def create_conditional_model(
    ...
    return ConditionalUNet(
        in_channels=1,  # 2 channels for bi-temporal input image s
        model_channels=num_channels,
        out_channels=1,  # Output for bi-temporal images
        num_res_blocks=num_res_blocks,
        attention_resolutions=tuple(attention_ds),
        channel_mult=channel_mult,
        num_heads=num_heads,
